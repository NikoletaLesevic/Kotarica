...
class AdListModel extends ChangeNotifier {
  ...
  getAds() async {
    ...
    debugPrint("Ad count: " + adCount.toString());
  ...
  getAdsFromIndex() async {
    ...
    debugPrint(ads.length.toString());
  

    for(var i = ads.length; i<ads.length + 13 && i < adCount; i++) {
      var temp = await _client
          .call(contract: _contract, function: _ads, params: [BigInt.from(i)]);
     
     debugPrint(addExists(temp[0], temp[5], ads.length).toString());
